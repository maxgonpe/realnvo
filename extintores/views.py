...
def odt_agregar_productos(request, pk):
    ...
    for detalle in odt.detalles.all():
        ...
        # 🔹 Detectar problemas normales
        for campo, nombre in componentes.items():
            if getattr(detalle, campo) is False:
                problemas.append({'campo': campo, 'nombre': nombre})

        # 🔹 Caso especial: extintor nuevo
        if getattr(detalle, "estado", "").lower() == "nuevo":
            problemas.append({'campo': 'estado', 'nombre': 'Extintor'})
...
def detalle_intervencion(request, pk):
    ...
    # --- Estadísticas específicas para PQS ---
    conteo_pqs_por_peso = Counter()
    conteo_co2_por_peso = Counter()
    conteo_agua_por_peso = Counter()
    conteo_k_por_peso = Counter()
    conteo_cf3_por_peso = Counter()
    conteo_afff_por_peso = Counter()
    total_pqs = 0
    total_co2 = 0
    total_k = 0
    total_cf3 = 0
    ...
    total_afff = 0
    for d in detalles:
        if d.agente and "pqs" in d.agente.lower():
            peso = d.peso or "N/A"
            conteo_pqs_por_peso[peso] += 1
            total_pqs += 1

    for d in detalles:
        if d.agente and "co2" in d.agente.lower():
            peso = d.peso or "N/A"
            conteo_co2_por_peso[peso] += 1
            total_co2 += 1

    for d in detalles:
        if d.agente and "tipo k" in d.agente.lower():
            peso = d.peso or "N/A"
            conteo_k_por_peso[peso] += 1
            total_k += 1

    for d in detalles:
        if d.agente and "cf3" in d.agente.lower():
            peso = d.peso or "N/A"
            conteo_cf3_por_peso[peso] += 1
         
            total_cf3 += 1

    for d in detalles:
        if d.agente and "agua" in d.agente.lower():
            peso = d.peso or "N/A"
            conteo_agua_por_peso[peso] += 1
            total_agua += 1

    for d in detalles:
        if d.agente and "afff" in d.agente.lower():
            peso = d.peso or "N/A"
            conteo_afff_por_peso[peso] += 1
            total_afff += 1
    ...
    for d in detalles:
        for campo, nombre in componentes.items():
            ...
            # 🔹 Agregar el caso especial de estado == "nuevo"
        if getattr(d, "estado", "").lower() == "nuevo":
            clave = (d.agente or 'N/A', d.peso or 'N/A', "Extintor Nuevo")
            resumen[clave] += 1
    ...
    return render(request, 'intervenciones/detalle_intervencion.html', {
        ...
        'conteo_agente_peso': dict(conteo_agente_peso),
        'conteo_pqs_por_peso': dict(conteo_pqs_por_peso),
        'total_pqs': total_pqs,
        'conteo_co2_por_peso': dict(conteo_co2_por_peso),
        'total_co2': total_co2,
        'conteo_agua_por_peso': dict(conteo_agua_por_peso),
        'total_agua': total_agua,
        'conteo_k_por_peso': dict(conteo_k_por_peso),
        'total_k': total_k,
        'conteo_cf3_por_peso': dict(conteo_cf3_por_peso),
        'total_cf3': total_cf3,
        'conteo_afff_por_peso': dict(conteo_afff_por_peso),
