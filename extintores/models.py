...
class EstadisticaMensual(models.Model):
    ...
    tipo = models.CharField(max_length=30)  # 'intervencion', 'odt', 'productos'

    # Para intervenciones
    cantidad_intervenciones = models.PositiveIntegerField(default=0)
    cantidad_extintores = models.PositiveIntegerField(default=0)
    tipo_intervencion = models.CharField(max_length=20, blank=True, null=True)  # 'revision', 'mantencion'...

    # Para ODTs
    cantidad_odt = models.PositiveIntegerField(default=0)

    # Para productos usados en ODT
    total_productos_utilizados = models.PositiveIntegerField(default=0)
    total_valor_productos = models.DecimalField(max_digits=12, decimal_places=2, default=0)

    # Filtros de análisis opcionales
    cliente = models.ForeignKey(Cliente, null=True, blank=True, on_delete=models.SET_NULL)
    categoria_producto = models.ForeignKey(CategoriaProducto, null=True, blank=True, on_delete=models.SET_NULL)
    ...
    def __str__(self):
        return f"{self.mes} - {self.tipo} - {self.tipo_intervencion or 'general'}"

class EstadisticaDetalleExtintor(models.Model):
    mes = models.CharField(max_length=7)  # Ej: '2025-07'
    tipo_intervencion = models.CharField(max_length=20)  # 'revision', 'mantencion', etc.
    ...
    def __str__(self):
        return f"{self.mes} - {self.tipo_intervencion} - {self.agente} - {self.peso} - {self.estado} ({self.cantidad})"


class EstadisticaDetalleProducto(models.Model):
    ...
    def __str__(self):
        return f"{self.mes} - {self.producto} - {self.cantidad} u"
